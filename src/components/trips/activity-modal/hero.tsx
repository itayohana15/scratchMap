...
/**
 * Hero image + up-to-10-photo gallery strip (spec: "Upgrade activity image
 * gallery"). One unified images[] + activeImageIndex — never separate
 * hero/thumbnail datasets. Fetches only while the modal is actually open
 * (usePlaceImages is gated by `open`), never preloaded for a whole trip.
 * A themed placeholder (pure theme tokens, no hardcoded color) covers both
 * "still loading" and "nothing found" so the modal never shows a broken
 * image.
 */
export function ActivityHero({ item, countryName, open }: ActivityHeroProps) {
  const placeImages = usePlaceImages(item.name, item.location || countryName, open);
  ...
  // If the active index drifted past the end (e.g. the active image just
  // failed and got filtered out), clamp back into range instead of showing
  // nothing.
  ...
  // Left/Right arrow navigation while the modal is open (spec item 8) —
  // skipped entirely while a text input/textarea/select is focused so it
  // never interferes with typing (e.g. the "הערות שלי" field).
  useEffect(() => {
    if (!open) return;
  ...
  const badges = [
    ...
    item.approximatePrice ? formatCurrency(item.approximatePrice) : "חינם",
    item.reservationRequired ? "דרושה הזמנה מראש" : null,
