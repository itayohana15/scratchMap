...
import type { GenerationStageState } from "@/lib/hooks/use-itinerary-generation-progress";
...
/** A generated itinerary counts as "over budget" only with a meaningful margin (§B14) — never a hair over. */
...
export type GenerationModalView = "pending" | "success" | "budgetWarning" | "error" | null;
...
export function AiGenerationProgressModal({
  ...
  useEffect(() => {
    if (view !== "pending") setConfirmingCancel(false);
  }, [view]);
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={(next) => {
        // §B9: while generation is running, X / outside-click / escape do nothing —
        // cancellation only happens through the explicit confirm flow below.
        if (isPending) return;
        if (!next) onClose();
      }}
    >
      <DialogContent
        ...
        className="max-w-[calc(100%-1.5rem)] overflow-hidden p-0 sm:max-w-lg motion-reduce:duration-0"
      ...
      >
        ...
        >
          ...
          <DialogHeader className="max-w-[85%] space-y-1.5">
            <DialogTitle id={titleId} className="text-xl sm:text-2xl">
              {view === "error"
          ...
          {view === "success" ? (
            <div className="mt-6 flex items-center gap-2 text-primary">
              ...
              <span className="font-medium">המסלול מוכן</span>
            </div>
          ) : null}
